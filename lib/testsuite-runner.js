...
module.exports.runner = function () {
    ...
    async function run() {
        ...
        if (test) {
            ...
            __logit(redisId + '   API Responded, has error ? ' + (test._apiResponse.err != undefined));
            test._response = test._apiResponse.status || test._apiResponse.err;
            if (test._apiResponse.err) {
                test.apiErr = true;
            }
            if (!test._apiResponse.err && test.output.type === 's3') {
